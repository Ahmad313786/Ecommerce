      ...
      <div>
        ...
        <div className="flex gap-3">
          ...
          <div onClick={()=> setSizes(prev => prev.includes("S") ? prev.filter(item => item !== "S") : [...prev, "S"])}>
            <p className={`${sizes.includes("S") ? "bg-rose-500 text-white" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>S</p>
          </div>
          {/* Medium */}
          <div onClick={()=> setSizes(prev => prev.includes("M") ? prev.filter(item => item !== "M") : [...prev, "M"])}>
            <p className={`${sizes.includes("M") ? "bg-rose-500 text-white" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>M</p>
          </div>
          {/* Large */}
          <div onClick={()=> setSizes(prev => prev.includes("L") ? prev.filter(item => item !== "L") : [...prev, "L"])}>
            <p className={`${sizes.includes("L") ? "bg-rose-500 text-white" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>L</p>
          </div>
          {/* XL */}
          <div onClick={()=> setSizes(prev => prev.includes("XL") ? prev.filter(item => item !== "XL") : [...prev, "XL"])}>
            <p className={`${sizes.includes("XL") ? "bg-rose-500 text-white" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>XL</p>
          </div>
          {/* 2XL */}
          <div onClick={()=> setSizes(prev => prev.includes("XXL") ? prev.filter(item => item !== "XXL") : [...prev, "XXL"])}>
            <p className={`${sizes.includes("XXL") ? "bg-rose-500 text-white" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>XXL</p>
          </div>
        </div>
      </div>
      {/* Best Seller */}
