      ...
      <div>
        {
          cartData.map((item, index) => {
            const productData = all_product.find((product) => product._id == item._id)
            return (
              <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4' >
                ...
                <img onClick={() => updateQuantity(item._id,item.size,0)} className='w-10 mr-4 sm:mr-5 cursor-pointer' src={bin} alt="" /> 
                {/* Day 4 4:00:37 */}
              </div>
            )
          })
