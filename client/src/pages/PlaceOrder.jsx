...
const PlaceOrder = () => {
    ...
    const options = {
      ...
      handler: async (res) => {
        ...
        try {
          ...
          const { data } = await axios.post(backendURL + '/api/order/verifyRazorpay',res,{"headers": { "Authorization": `Bearer ${token}`}} )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            const itemInfo = structuredClone(
              all_product.find((product) => product._id == items)
            );
            if (itemInfo) {
              itemInfo.size = item;
      ...
      switch (method) {
        ...
        case "razorpay":
          const resRazorpay = await axios.post(backendURL + '/api/order/razorpay', orderData, {"headers": { "Authorization": `Bearer ${token}`}})
  ...
  return (
    <form
      ...
      className="flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-hi-[80vh] border-t "
    >
      {/* ----------- Left Side ---------------- */}
